...
import com.example.cloud.View.EditUriCli;
import com.example.cloud.View.EditPhysCli;
...
public class Client {
    public static Pane getPane(int fl) throws SQLException {
        ...
        ArrayList<UriCli> mas = Postgre.getAllLegalClient();
        int u = 80;
        for(int i=0; i<mas.size();i++, u+=70){
            ...
            edit.setOnAction(t->{
                Front.root.getChildren().remove(Front.pane);
                try {
                    Front.pane = EditUriCli.editLegal(id,fl);
                } catch (SQLException e) {
                    throw new RuntimeException(e);
                }
                Front.root.getChildren().add(Front.pane);
        ...
        ArrayList<PhysCli> mas2 = Postgre.getAllPhysClient();
        for(int i=0; i<mas2.size();i++, u+=70){
            ...
            edit.setOnAction(t->{
                try {
                    Front.root.getChildren().remove(Front.pane);
                    Front.pane = EditPhysCli.editPhys(id,fl);
                    Front.root.getChildren().add(Front.pane);
                } catch (SQLException e) {
                    throw new RuntimeException(e);
                }
    ...
    public static Pane getPane2(int fl, String el) throws SQLException {
        Pane pane = new Pane();
        Text num_text = new Text("#");
        num_text.setLayoutX(20);
        num_text.setLayoutY(60);
        ...
        ArrayList<UriCli> mas = Postgre.getAllLegalClient2(el);
        int u = 80;
        for(int i=0; i<mas.size();i++, u+=70){
            TextField num = new TextField();
            num.setEditable(false);
        ...
        ArrayList<PhysCli> mas2 = Postgre.getAllPhysClient2(el);
        for(int i=0; i<mas2.size();i++, u+=70){
            TextField num = new TextField();
            num.setEditable(false);
            num.setStyle("-fx-background: transparent; -fx-background-color: transparent;");
